#' Generate ISO-class object containing isotope data, metadata, and other separators.
#'
#' @param x text
#'
#' @usage
...
ISOgen <- function(data,
                   data.names = NULL,
                   seperator = NA,
                   doi, dfi, lambda) {
    requireNamespace("tidyverse", quietly = T)
    # Parse different isotope data; separate into lists for appending
    if (is_tibble(data)) {
        isodata <- data %>%
